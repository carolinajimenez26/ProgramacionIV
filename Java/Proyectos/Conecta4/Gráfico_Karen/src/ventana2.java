...
public class Imagen extends javax.swing.JPanel{
    ...
    int x,y;//posiciones donde está el objeto (x->columna, y->fila)
    ImageIcon img;//clase de java, para poner las imagenes dentro del panel
 
    public Imagen(int x,int y,ImageIcon img) {//recibe la posicion donde va a ir la imagen
        this.setSize(280, 240); //tamaño total del panel sobre el que vamos a dibujar y trabajar
        this.x = x;
        this.y = y;
        this.img = img;
    }
    ...
    public void paint(Graphics grafico) {//se llama automáticamente

        //se dibuja la imagen que tenemos en el paquete Images //dentro de un panel

        grafico.drawImage(img.getImage(), x*40, y*40,img.getIconWidth(),img.getIconHeight(), null);
        //Le enviamos la imagen que queremos colocar, sus atributos y dónde debe quedar
        setOpaque(false);
        super.paintComponent(grafico); //lo pinta
    }
}  
    
    private void MoverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MoverActionPerformed
        
        ImageIcon imagen1= new ImageIcon("circulo-amarillo.png");//imagenes que vamos a utilizar
        ImageIcon imagen2= new ImageIcon("circulo-azul.png");
        
        int col = Integer.parseInt(columnaIngresada.getText());//recibe la columna(lo ingresa el usuario)
        ...
        if(T.Finish()){
            ...
            T.CambiarColor(2);//juega la máquina
            int r = minimax.minimax(T);//saca el movimiento
            if(r !=-1) T.Pinta(r);//lo pinta
            
            Imagen Imagen2 = new Imagen(T.Columna(),T.Fila(),imagen2);//carga la imagen
            iconos.add(Imagen2);//pone la imagen
            
            iconos.repaint();//repinta
            
            iconos.add(Imagen1);
            
            if(T.Finish()){//si ganó la máquina
                
                ventana=new ganaOpierde(2);
                iconos.removeAll();//remueve todo lo que había puesto
                this.setVisible(false);
            }
...
class Figuras extends JPanel{
        ...
        public void Figuras(Graphics g){
            ...
            ImageIcon imagen1 = new ImageIcon("circulo-amarillo.png");//carga la imagen
